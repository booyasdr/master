...
void WEBSOCKET_generate_handshake( const char *data, char *dst, const unsigned int dst_len ) {
	...
	for( i = 0; i < 5; i++ ) {
		...
		int l = strlen(sha1_part);
		if(l!=8) {
//		snprintf( sha1_part, 32, "%8x", sha.Message_Digest[i] );
		    char temp[32] = {'\0'};
            int b = (8-strlen(sha1_part));
            for(int n = 0; n < b;n++) strcat(temp,"0");
            strcat(temp,sha1_part);
            strcpy(sha1_part,temp);
            printf("FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX \n");
            printf("FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX FIX \n");
		}
		strncat( (char *)sha1_tmp, sha1_part, 512 );

	}

	strncpy( sec_websocket_key_sha1, (char *)sha1_tmp, 512 );
	source_len = xstr2str( (char *)sha1_hex, 512, sec_websocket_key_sha1 );
	base64_encode( sha1_hex, source_len - 1, (char *)sec_websocket_accept, 512 );
...
#define NMESSAGE 6
unsigned char *pmessage[NMESSAGE] = {NULL};
int imess = 0;
...
//	unsigned char *message = ( unsigned char * )malloc( 65535 * sizeof( char ) );
   if(!pmessage[0]) for(int n; n < NMESSAGE; n++) pmessage[n] = ( unsigned char * )malloc( 65535 * sizeof( char ) );
   unsigned char *message = pmessage[imess];
   imess = (imess+1)%NMESSAGE;
	int i;
...
//	char *connection_header = ( char * )malloc( 64 * sizeof( char ) );
	char connection_header[64 * sizeof( char ) ];
	short result = 0;
...
//	char *version_header = ( char * )malloc( 32 * sizeof( char ) );
	char version_header[32 * sizeof( char ) ];
	int result;
