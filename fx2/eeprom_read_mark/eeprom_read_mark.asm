...
; This file was generated Sun Apr 26 11:08:00 2015
;--------------------------------------------------------
	.module eeprom_read_mark
...
;	eeprom_read.h:120: xdata unsigned char *HWmark = (xdata unsigned char *)0x1140;
	mov	_HWmark,#0x40
	mov	(_HWmark + 1),#0x11
;	eeprom_read_mark.c:19: char *FWmark = "FWEEPRv000.001";
	mov	_FWmark,#__str_2
	mov	(_FWmark + 1),#(__str_2 >> 8)
	mov	(_FWmark + 2),#0x80
	.area GSFINAL (CODE)
...
;------------------------------------------------------------
;	eeprom_read_mark.c:45: void main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	eeprom_read_mark.c:51: err = (xdata unsigned char *) 0x1200;	
	mov	_err,#0x00
	mov	(_err + 1),#0x12
;	eeprom_read_mark.c:53: for(n = 0;n < 200;n++);
	mov	r6,#0xC8
	mov	r7,#0x00
00103$:
	dec	r6
...
00120$:
	...
	orl	a,r7
	jnz	00103$
;	eeprom_read_mark.c:54: readmarkeeprom();
	lcall	_readmarkeeprom
;	eeprom_read_mark.c:55: for(n=0;n<32;n++) dat[n] = (xdata char)HWmark[n];
	mov	r6,#0x00
	mov	r7,#0x00
00104$:
	clr	c
	...
	sjmp	00104$
00107$:
;	eeprom_read_mark.c:57: hostreadmarkep1init();
	lcall	_hostreadmarkep1init
00109$:
;	eeprom_read_mark.c:58: for(;;) hostreadmarkep1poll();	
