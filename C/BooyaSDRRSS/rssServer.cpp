...
rssServerClass rssServer;

void* rssServerClass::listener(void *args) { // listen for new connections

    class rssServerClass *prssServer = (class rssServerClass *) args;
    class listenerInputClass *pinput = (class listenerInputClass *) &prssServer->listenerInput;
...
//     bind() associates a local address and port combination with the socket just created.
    ...
    while(1) {
        ...
        prssServer->newConnection(msgsock,&from);
...
rssConnectionClass* rssServerClass::newConnection(SOCKET msgsock,struct sockaddr_in *pfrom) {
   bool found = false;
   rssConnectionClass *pc;
   int n;
...
void rssServerClass::closeConnection(rssConnectionClass* pc) {
   pc->isOpen = false;
...
//void *rssServerClass::serverStatus(void *args) {
//   rssServerClass *p = (rssServerClass *) args;
...
//   while (1) {
//      Sleep(2000);
//RSSSERVERMUTEXCONNECTIONLIST_LOCK
//      int M = p->nConnection;
...
//         }
//      }
//RSSSERVERMUTEXCONNECTIONLIST_UNLOCK
//	}
//	pthread_exit(NULL);
//	return NULL;
//}
