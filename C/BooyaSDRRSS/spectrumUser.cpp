...
RSSSERVERMUTEXCONNECTIONLIST_LOCK
		for(int m = 0; m < rssServer.nConnection; m++) {
         if(rssServer.connectionList[m]->state > notConnected) {
            rssConnectionClass *pc = rssServer.connectionList[m];
            if(pc->isOpen) pc->RSSsendData( outloop );
         }
      }
