...
The time call has the variables:
    spectrum.idx - spectum action index
    spectrum.period - spectrum action period
The time call does the following:
    increment spectrum.idx modulo spectrum.period
    on spectrum.idx == 0, call spctrumf() to do the fft
    on spectrum.idx == 3, update image buffers with the resulting data
*/
...
spectrumFrame::spectrumFrame(wxWindow* parent,wxWindowID id)
{
	...
	SetClientSize(wxSize(1040,800));
	Move(wxPoint(10,800));
	mainSpectrumPanel = new wxPanel(this, ID_SPFRPANEL1, wxPoint(0,20), wxSize(1040,500), 0, _T("ID_SPFRPANEL1"));
	...
	Menu6 = new wxMenu();
	Widthx1 = new wxMenuItem(Menu6, ID_MENUITEM23, _("Single"), wxEmptyString, wxITEM_RADIO);
	Menu6->Append(Widthx1);
	Widthx2 = new wxMenuItem(Menu6, ID_MENUITEM24, _("Double"), wxEmptyString, wxITEM_RADIO);
    ...
    halfFftSp1024 = round(g.fs/2./1000./500.);  // init spectrum global
    khpb500->Check();

    NyquistKHz      = g.fs/1000/2;                     // NyQuist rate in kHz
    kHzPerTile      = NyquistKHz/halfFftSp1024;             // Max Frequency over number of 1024 panels
    kHzPerPix       = float(kHzPerTile)/1024.;              // Panel kHz divided by 1024 pixels per panel
...
// Initialize menu settings
    localRadioUser.AGC.on = true;
    AMAGCon->Check();
    localRadioUser.AGC.on = false;
    AMAGCoff->Check();
...
// Initialize control settings
   ...
   frequencySlider->SetValue(localRadioUser.tunekHz);
   gainSlider->SetValue(localRadioUser.gain);
...
// Initialize tile settings (a tile is 1024 wide by 90 high)
    tileSize =  wxSize(1024,90);
    windowSize = wxSize(16 + nHorTile*tileSize.x, menubar+wtopmargin+pheight*nVertTile+pbottommargin);

// create tiles
    for(int n = 0; n<N_VERT_TILE_MAX*N_HOR_TILE_MAX;n++) {
		ID_TILE[n] = wxNewId();
        Tile[n]= new wxPanel(mainSpectrumPanel, ID_TILE[n], wxPoint(6,wtopmargin + n*pheight), tileSize,
...
// initialize frequency labels
    ...
    for(int n = 0; n < N_VERT_TILE_MAX*N_HOR_TILE_MAX*
        ...
        frequencyLabel[n] = new wxStaticText(mainSpectrumPanel, ID_FREQUENCY_LABEL[n],
                s[n], wxPoint(6+x,wtopmargin + (pan+1)*pheight-13),
    ...
    for(int n = 0, kHz = 0; n < N_VERT_TILE_MAX*N_HOR_TILE_MAX*
        ...
        int x = 6 + int(float(int(kHz)%int(kHzPerTile*nHorTile))/kHzPerPix);
...
// end initialize spImageBuffer
   ...
   char title[256];
	sprintf(title,"BooyaSDRWeb ""%s %d MHz Spectrum Window",g.execName,(int)round(g.fs/1e6));
	SetTitle(title);
...
// setup time window
    ...
    TimeFrame->Show();
	sprintf(title,"Booya %d MHz Time Sample Window",(int)round(g.fs/1e6));
...
// paint panel
    ...
    wxImage pWxImg = wxImage(tileSize.x,tileSize.y,image, TRUE);
    wxBitmap bmp = wxBitmap(pWxImg);
    dc.DrawBitmap(bmp, 0,0,false);

// find the tune frequency panel
//    int tunePanel = localRadioUser.tunekHz/kHzPerTile-firstBuffer;
    int tunePanel = localRadioUser.tunekHz/kHzPerTile;
    tunePanel -= firstBuffer;
    if( n == tunePanel ) {
        wxColor Magenta(255,0,255);
        wxPen myMagentaPen(Magenta,2,wxSOLID);
        dc.SetPen(myMagentaPen);
        int x = ((int)localRadioUser.tunekHz%kHzPerTile)/kHzPerPix;
...
void spectrumFrame::initImageUpdateThread(void) {
    ...
    if(g.gui==false) return;

    int rc = pthread_create(&image_update_thread[tt], NULL, imageUpdatef, (void *)&tt);
    if (rc)  printf("ERROR; return code from pthread_create() imageUpdateThreadf is %d\n", rc);
}
...
void *imageUpdatef(void *arg)
{
   while (true) {
      ...
      unsigned short *plotdata
         = spectrum.nextLocalOut();   // get output of spectrumf()

      // update all the image buffers
      for( int o=0; o < halfFftSp1024; o++) {
         // record the last line
         rgb *imb =(rgb *) sf->spImageBuffer[o];
         //       for (int n = 0, m = sf->imageLineIdx *sf->tileSize.x*3; n< sf->tileSize.x ; m+=3, n++) {
         for (int n = 0, m = sf->imageLineIdx *sf->tileSize.x ; n< sf->tileSize.x ;  n++, m++) {
         ...
         memcpy(&sf->spImageBuffer[o][(sf->imageLineIdx+sf->tileSize.y) *sf->tileSize.x*3],
             &sf->spImageBuffer[o][sf->imageLineIdx *sf->tileSize.x*3],  sf->tileSize.x*3);
         // increment data source pointer for next buffer
         plotdata += sf->tileSize.x;
      }
      ++sf->imageLineIdx %= sf->tileSize.y;
      // set the image pointer for painting the panel
      for( int o=0; o < sf->nVertTile*sf->nHorTile; o++) {
      sf->pImage[o] = &sf->spImageBuffer[o + sf->firstBuffer][sf->imageLineIdx *sf->tileSize.x*3];
...
void spectrumFrame::OnTileMouseDown(wxMouseEvent& event) {
    ...
    int x = event.GetX();
    localRadioUser.tunekHz = int(((n+firstBuffer)*kHzPerTile + x*kHzPerPix + float(radioTuneInc)/2)/radioTuneInc)*radioTuneInc;
    localRadioUser.ffHz = 0;
//    printf("panel %d, x %d, y %d tunekHz %f\n",n,x,y,localRadioUser.tunekHz);
    frequencySlider->SetValue(localRadioUser.tunekHz);
    fineFrequencySlider->SetValue(localRadioUser.ffHz);
...
void spectrumFrame::makeMark(wxPaintDC *pdc, Mark *pmark, wxColor *pcolor, int kHzinc) {
    ...
    for (int kHzS = pmark->kHzS,kHzE = pmark->kHzE;
        ...
        int xstart = 6 + int(float(kHzS%kHzinc)/kHzPerPix);
        int xend   = 6 + int(float((kHzE-1)%kHzinc)/kHzPerPix);
...
void spectrumFrame::OnfrequencySliderCmdScrollChanged(wxScrollEvent& event)
{
    ...
    localRadioUser.tunekHz = event.GetPosition();

    int pageSize = frequencySlider->GetPageSize();
    localRadioUser.tunekHz = (int(localRadioUser.tunekHz)/pageSize)*pageSize;
    frequencySlider->SetValue(localRadioUser.tunekHz);
...
// zero fine frequency
    ...
    if(SLIDER_DEBUG) debugSlider(dt,event,frequencySlider,localRadioUser.tunekHz);
}
...
void spectrumFrame::OnfrequencySliderCmdScrollPageUp(wxScrollEvent& event)
{
    ...
    if (dt < dckick)
        ...
        localRadioUser.tunekHz-=frequencySlider->GetPageSize();

    int maxi = frequencySlider->GetMax();
    int mini = frequencySlider->GetMin();
    if(localRadioUser.tunekHz > maxi) localRadioUser.tunekHz = maxi;
    if(localRadioUser.tunekHz < mini) localRadioUser.tunekHz = mini;
...
void spectrumFrame::OnfineFrequencySliderCmdScrollChanged(wxScrollEvent& event) {
    localRadioUser.ffHz = event.GetPosition();

    if(SLIDER_DEBUG) debugSlider(-1,event,brightnessSlider,localRadioUser.ffHz);
}
...
void spectrumFrame::OnfineFrequencySliderCmdScrollPageUp(wxScrollEvent& event)
{
    ...
    if(localRadioUser.ffHz < mini) localRadioUser.ffHz = mini;
    fineFrequencySlider->SetValue(localRadioUser.ffHz);

    if(SLIDER_DEBUG) debugSlider(dt,event,fineFrequencySlider,localRadioUser.ffHz);
}
...
void spectrumFrame::OnfineFrequencySliderCmdScrollPageDown(wxScrollEvent& event)
{
    ...
    if(localRadioUser.ffHz > maxi) localRadioUser.ffHz = maxi;
    fineFrequencySlider->SetValue(localRadioUser.ffHz);
...
void spectrumFrame::OngainSliderCmdScrollChanged(wxScrollEvent& event) {
    localRadioUser.gain = event.GetPosition();

    if(SLIDER_DEBUG) debugSlider(-1,event,gainSlider,localRadioUser.gain);
}
...
void spectrumFrame::OngainSliderCmdScrollPageUp(wxScrollEvent& event) {
    ...
    if(localRadioUser.gain < mini) localRadioUser.gain = mini;
    gainSlider->SetValue(localRadioUser.gain);
//    scaleRadiof(localRadioUser.gain);

    if(SLIDER_DEBUG) debugSlider(dt,event,gainSlider,localRadioUser.gain);
}
...
void spectrumFrame::OngainSliderCmdScrollPageDown(wxScrollEvent& event) {
    ...
    if(localRadioUser.gain > maxi) localRadioUser.gain = maxi;
    gainSlider->SetValue(localRadioUser.gain);
//    scaleRadiof(localRadioUser.gain);
...
void spectrumFrame::OnkHzPerTile(float resolutionkHzper1024) {
    ...
    int halfFft = round(g.fs/2./1000./resolutionkHzper1024);
    spectrum.sizeSpectrumf(halfFft); // halfFftSp1024 is set to halfFft in this function
...
// resize window
    windowSize.x = 16 + nHorTile*tileSize.x;
