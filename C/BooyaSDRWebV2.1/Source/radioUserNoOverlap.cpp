...
//           header[m].dwBufferLength = bbLen*sizeof(short)/2; // halve for overlap
           header[m].dwBufferLength = bbLen*sizeof(short); // halve for overlap
...
//        }
        bool gotoIF = false;
        tuneHz = ru->tunekHz*1000+ru->ffHz*100;
...
//        if((ru->blockIdx + fftPerBuf) >= blockLength) { // tuneBufIn will be filled
        if((ru->blockIdx + fftPerBuf) >= tuneLength) { // tuneBufIn will be filled
            gotoIF = true;
//            lim0 = blockLength - ru->blockIdx;
            lim0 = tuneLength - ru->blockIdx;
            IFbuf = ru->tbi0;
            ru->tbi0 = ru->tbi1;
            ru->tbi1++;
            ru->tbi1 %= N_IF_BUF;
...
//        ru->blockIdx = (ru->blockIdx+fftPerBuf)%blockLength;
        ru->blockIdx = (ru->blockIdx+fftPerBuf)%tuneLength;
    ...
    // Third stage, IFFT soudnBufIn
        ...
        if( mode == AM ) {
            ...
            if(ru->AGC.on) { // AGC
               ...
               Amag = ru->AGC.scale*SF;            // local
               if(pc) Amag = ru->AGC.scale*SF/4;   // web
               Aoffset = - AMlevel;
            }
        } else { // SSB or CW
            Amag = 8*ru->AGC.scale*SF;             // local
            if(pc) Amag = 4*8*ru->AGC.scale*SF;    // web
