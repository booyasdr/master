...
//    for(src=0xf000,dest = 0x1000;(int)dest < 0x1000;src++,dest++) *dest = (unsigned char) 0xa5;
}
...
void main()
{
 	...
 	Initialize();
 for(dest = 0x3000;dest <= 0x3fff;dest++) *dest = '\0';
...
x = 'z';
   ...
   for(dest =0xf000,i=0; i <= 0x10000 ;dest++,i++) *dest = x;
 for(dest = 0x3000;dest <= 0x3fff;dest++) *dest = (unsigned char) 0xaa;


 for(src = 0xf000,dest = 0x3000;(int)dest <= 0x3fff;dest++,src++)
       *dest = *src;
